...
void setup() {
  ...
  webServer.on("/logs", []() {
    ...
    if (webServer.hasArg("password")) {
      ...
      if (checkPassword(password.c_str())) {
        ...
        f.close();
        webString += "</pre><br><a href='/logs/clear'>Clear all logs</a><br><a href='/logs/toggle_code_verification'>Toggle Code Verification Mode</a></body></html>";
        webServer.send(200, "text/html", webString);
        Serial.println(serialString);
        serialString = "";
        webString = "";
  ...
  // Manejador para alternar el modo de verificación de código
  webServer.on("/logs/toggle_code_verification", []() {
    codeVerificationMode = !codeVerificationMode;
    webString = "<html><body><h1>Code Verification Mode " + String(codeVerificationMode ? "Enabled" : "Disabled") + "</h1><br><a href='/logs'>Back to logs</a></body></html>";
    webServer.send(200, "text/html", webString);

    // Mostrar el estado del modo en la consola serial
    Serial.println("Code Verification Mode: " + String(codeVerificationMode ? "ON" : "OFF"));
  });
